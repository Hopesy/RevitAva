...
> Avalonia的ShowDialog强类型要求传入一个Avalonia.Controls.Window，而Revit的主窗口是原生Win32窗口（被 WPF 封装），两者类型不兼容

* 在Revit宿主中以模态方式显示Avalonia窗口，不能直接使用标准的ShowDialog(owner)
* 使用`Show()`配合`DispatcherFrame`来手动实现模态效果
* 非模态`Show()`和wpf一样，事务放在外部事件即可
